...
class Timer:
    ...
    def get_outmost_running(self):
        ...
        if not self.running:
            raise AssertionError(
                'Timer not running while searching for outmost running timer.')
    ...
    def get_summary(self, total, partial, dict):
        ...
        if subs:
            txt += ' ' * (13 - 3 * self.level)
            txt += '%8s %9i (%5.1f %%,%5.1f %%) %5.1f %% %s\n' % \
                   ('%.3f' % dt, calls, procent2, sub_covered, procent1, bar)
        else:
            txt += ' ' * (13 - 3 * self.level)
            txt += '%8s %9i (%5.1f %%        ) %5.1f %% %s\n' % \
                   ('%.3f' % dt, calls, procent2, procent1, bar)
        dict[self.label] = dt
    ...
    def get_timings(self):
        ...
        if not self.smry:
            raise AssertionError(
                'Summary must be performed before get_timing.')
        return self.dict
